...
def select_criteria():
    default = [
        (
            "Paralax search condition. Format is operator, whitespace and value "
            "(e.g.'> 0.2').",
            "parallax",
        ),
        (
            "G magnitude search condition. Format is operator, whitespace and value "
            "(e.g.'< 18').",
            "phot_g_mean_mag",
        ),
    ]
...
def add_calculated_columns(df):
    ...
    df["e_BPRP"] = (
        2.5
        * np.log10(np.e)
        * np.sqrt((dfluxBP / fluxBP) ** 2 + (dfluxRP / fluxRP) ** 2)
    )
